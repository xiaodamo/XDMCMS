...
class Friends extends CI_Controller
{
    ...
    function index()
    {
        ...
        //总条数
        $counts =  $this->friends_model->counts($condition);
		$friendss = $this->friends_model->finds($condition,$per_page, ($p-1)*$per_page);
	...
	function edit()
    {
        ...
        if (!empty($params['id']) && $params['id'] > 0){
            $id = $params['id'];
            $this->load->model('friends_model');
            $data['editing'] = $this->friends_model->load($id);
            if (!$data['editing']){
		...
		$this->load->view('friends/add',$data);
	}
    ...
    {		
		...
		if (TRUE == $this->form_validation->run()){
            ...
            if ($id){
                ...
                $res = $this->friends_model->update($data);
            ...
            // 添加新管理员
               ...
               $res = $this->friends_model->create($data);
			   if($res){ 
                   show_message3('操作成功');
			   }else{
                   show_message3('操作成功','操作失败',false);
    ...
    function display()
    {
        ...
        if ($id > 0){
            $this->load->model('friends_model');
            $res = $this->friends_model->update_one(array('is_display'=>$is_display),$id);
    ...
    {
        $this->form_validation->set_rules('name', '友情链接名', 'trim|required');
