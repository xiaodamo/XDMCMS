...
class Base_Model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        if($this->_database){
            if(is_string($this->_database)){
                ...
                $this->_database = $this->__CI->load->database($this->_database,TRUE);
            }
        }else{
    ...
    public function insert($data, $skip_validation = FALSE)
    {
        ...
        if ($data !== FALSE)
        {
            ...
            $insert_id = $this->auto_increment?$this->_database->insert_id():$this->_database->affected_rows();
    ...
    public function validate($data)
    {
        ...
        if(!empty($this->validate))
        {
            ...
            if(is_array($this->validate))
            {
                ...
                {
                    show_message3('操作成功',$this->form_validation->error_string(),false);
                    //return FALSE;
                }
            }
            else
     ...
     * 自定义sql查询，表前缀用__表示
     * @param string $sql
     * @param array $array，传此参数时可以用来替换数据到?占位符
     * @return null
     */
    public function query_sql($sql = "",$array = array())
    {
        if(!$sql) return null;
        $sql = str_replace('__',$this->_database->dbprefix,$sql);
        return $this->_database->query($sql,$array)->{$this->_return_type(1)}();
    }
     ...
     * @return boolean
     */
    public function do_sql($sql = "",$array = array())
    {
        if(!$sql) return null;
        $sql = str_replace('__',$this->_database->dbprefix,$sql);
